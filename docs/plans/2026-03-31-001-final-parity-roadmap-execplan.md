...
- [ ] M1: stop fleet redispatch/spawn/backlog feedback loops and prove deterministic native/WASM replay across five 5,000-tick seeds. F-007A prevents redispatch and travel resets. F-007B closes stale position, production attachment, and compatible arrival accumulation. F-007C closes ordinary player dispatch. Next reduce transit churn, distribute system combat, enable victory, and extend cross-runtime replay to all five seeds.
...
- Observation: F-007B removed unbounded fleet-record growth and exposed a separate player-path gap plus concentrated combat behavior. F-007C closes the player path.
  Evidence: `pending_move_destination` now reaches a validated `DispatchFleet` action and the shared departure helper. Five 5,000-tick runs still leave 62.5 to 100 percent of fleets in transit, battles occur in at most two systems, and no run reaches victory.
