...
- [ ] M1: stop fleet redispatch/spawn/backlog feedback loops and prove deterministic native/WASM replay across five 5,000-tick seeds. F-007A prevents redispatch and travel resets. F-007B closes stale position, production attachment, and compatible arrival accumulation. F-007C closes ordinary player dispatch. F-007D closes the system combat backlog. F-007E next balances AI encounters, transit fan-in, repair telemetry, and victory before five-seed cross-runtime replay.
...
- Observation: F-007D removed the permanent combat backlog and exposed a separate AI campaign-balance failure.
  Evidence: seed 42 now resolves Xyquine once instead of reporting 602 recurring draws. Across five current runs, only zero to six engagements occur, seeds 1,337 and 424,242 exceed the fleet-arena bound, and no run reports victory.
