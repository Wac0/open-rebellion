...
| M5 — Continuous proof | Enforce format/clippy/build/browser checks; short and long campaign gates; resource and screenshot ledgers; app integration tests; package boot and data/save hashes. | Every supported P00–P38 pass is green from release artifacts, with reproducible evidence retained by CI. |
...
- Every supported feature ID is `pass` on every claimed platform.
